...
class BaseBuffer(ABC):
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None):
        ...
        # print(f"BaseBuffer sample(): batch_size: {batch_size}, upper_bound: {upper_bound}, batch_inds: {batch_inds}")
...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        if psutil is not None:
            ...
            # mem_available /= 1e9
            # print(f"replay buffer {total_memory_usage:.2f}GB > {mem_available:.2f}GB")
    ...
    def add(
        ...
        # print("self.observations[self.pos] shape:", self.observations[self.pos].shape)
        # raise Exception("debug replaybm.py")
    ...
    def save_buffer(self, filepath: str) -> None:
        ...
        :param filepath: Path where the buffer will be saved
        """
        import pickle
        ...
        # Only save next_observations if not using memory optimization
        if not self.optimize_memory_usage:
            buffer_data['next_observations'] = self.next_observations
    ...
    def load(self, filepath: str) -> None:
        ...
        :param filepath: Path to the saved buffer file
        """
        import pickle
        ...
        # Restore next_observations if it exists
        if 'next_observations' in buffer_data and not self.optimize_memory_usage:
