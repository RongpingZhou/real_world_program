...
def process_serial_data():
    ...
    received_data = data_queue.get()  # Retrieve data from the queue
...
def main():
    ...
    if args.actuator == 1:
        port = '/dev/ttyUSB0'  # serial port number based on the system setting
        baudrate = 57600  # baud rate for hardware emulated keyboard
    ...
    elif args.actuator == 2:
        port = '/dev/ttyUSB0'  # serial port number based on the system setting
        baudrate = 19200  # baud rate for physical keyboard clicker
        ...
        # while episodes < 3:
            ...
            if args.actuator != 0:
                if args.actuator == 1:
                    ...
                    while terminated or truncated:
                        if not data_queue.empty():
                            print(f"Start all episodes: Env reset inside Noop reset: Lives: {lives}, no action")
    ...
    if args.sensor == 0 or (args.sensor == 1 and args.actuator == 0):
        print(f"steps: {steps}")
        print(f"info: {info}")
        pygame.quit()
