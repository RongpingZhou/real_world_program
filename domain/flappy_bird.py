...
class FlappyBirdEnv(gym.Env):
    ...
    def __init__(self, FPS: int = 10, render_mode: Literal["human", "rgb_array"] | None = None,):
        ...
        pygame.init()
        # If you only need the display module (without initializing other modules), 
        # you can call pygame.display.init() directly instead of pygame.init().
        # pygame.display.init()
        self.window = pygame.display.set_mode((self.screenwidth, self.screenheight), pygame.HIDDEN)
        ...
        """
        self._actions = {
            Actions.NO_FLAP.value: np.array([1, 0]),
            Actions.FLAP.value: np.array([0, 1]),
        }
    ...
    def _get_obs(self):
        ...
        #     )
        # image_data = np.transpose(np.array(pygame.surfarray.pixels3d(self.canvas)), axes=(1, 0, 2))
        # image_data = np.array(pygame.surfarray.pixels3d(self.canvas))[:, :, [2, 1, 0]]
        # print(f"image_data shape: {image_data.shape}")
        # image_data = np.rot90(np.flip(np.array(pygame.surfarray.pixels3d(self.canvas))[:, :, [2, 1, 0]], axis=1))
        
        # pygame surface size is W x H, but we need H x W for the observation space
    ...
    def _render_frame(self):
        ...
        # else:  # rgb_array
        #     # return np.array(pygame.surfarray.pixels3d(self.canvas))
        #     return np.transpose(
        #         np.array(pygame.surfarray.pixels3d(self.canvas)), axes=(1, 0, 2)
        #     )
