...
def play(
    ...
    while game.running:
        if done:
            ...
            # print(f"Key pressed: ")
            # for key in game.pressed_keys:
            #     print(f"{pygame.key.name(key)}")
            # print("end of the keys pressed")
            action = key_code_to_action.get(tuple(sorted(game.pressed_keys)), noop)
            prev_obs = obs
            obs, rew, terminated, truncated, info = env.step(action)
            done = terminated or truncated
            if callback is not None:
                callback(prev_obs, obs, action, rew, terminated, truncated, info, game.running)
        if obs is not None:
            rendered = env.render()
            if isinstance(rendered, List):
                rendered = rendered[-1]
