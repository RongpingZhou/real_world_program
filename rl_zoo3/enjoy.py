...
def enjoy() -> None:  # noqa: C901
    ...
    print(f"*****hyperparams: {hyperparams['env_wrapper']}")
    ...
    obs = env.reset()
    # print(f"obs.shape: {obs.shape}, dtype: {obs.dtype}, device: {obs.device}")ls
    ...
    try:
        for _ in generator:
            ...
            # print(f"lstm_states: {lstm_states}, action: {action}")
            obs, reward, done, infos = env.step(action)
            # print(f"*****obs.shape: {obs.shape}, type: {type(obs)}, device: {obs.device}")
