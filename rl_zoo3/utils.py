...
def get_wrapper_class(hyperparams: dict[str, Any], key: str = "env_wrapper") -> Optional[Callable[[gym.Env], gym.Env]]:
    ...
    if key in hyperparams.keys():
        ...
        def convert_shape(shape: tuple[int, int]):
            ...
            """
            print(f"abc called with shape: {shape}")
            return shape
        ...
        def wrap_env(env: gym.Env) -> gym.Env:
            print(f"Wrapping environment with {wrapper_classes}")
            print(f"and kwargs {wrapper_kwargs}")
            for wrapper_class, kwargs in zip(wrapper_classes, wrapper_kwargs):
                print(f"Applying wrapper {wrapper_class} with args {kwargs}")
                if wrapper_class is gym.wrappers.transform_observation.ResizeObservation:
                    shape = convert_shape(**kwargs)
                    shape = tuple([int(shape), int(shape)])
                    print(f"Resizing observation to shape: {shape}")
                    env = wrapper_class(env, shape)
                else:
                    env = wrapper_class(env, **kwargs)
            return env
...
def create_test_env(
    ...
    print(f"Creating test environment {env_id} with the following kwargs: {env_kwargs}")
...
def get_hf_trained_models(organization: str = "sb3", check_filename: bool = False) -> dict[str, tuple[str, str]]:
    ...
    for model in models:
        ...
        # print(f"model.card_data: {type(model.card_data)}")  # DEBUG
        # print(f"model: {model}")  # DEBUG
        # print(f"model.card_data: {model.card_data}")  # DEBUG
        # print(f"model.card_data model name: {model.card_data['eval_results']}")  # DEBUG
        try:
            ...
            # print(f"Exception occurred while parsing model card: {e}")
