        ...
        def _init() -> gym.Env:
            ...
            if isinstance(env_id, str):
                ...
                try:
                    ...
                    print(f"make_vec_env with kwargs: Creating environment {env_id} with the following kwargs: {kwargs}")
                except TypeError:
                    env = gym.make(env_id, **env_kwargs)
                    print(f"make_vec_env with env_kwargs: Creating environment {env_id} with the following kwargs: {kwargs}")
            else:
                env = env_id(**env_kwargs)
                # Patch to support gym 0.21/0.26 and gymnasium
                env = _patch_env(env)
                print(f"make_vec_env with _patch_env: Creating environment {env_id} with the following kwargs: {env_kwargs}")
            if seed is not None:
